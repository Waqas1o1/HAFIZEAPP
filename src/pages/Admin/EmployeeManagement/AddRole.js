...
const AddRole = () => {
    ...
    async function fetchRole(){
        return await axiosInstance.get('apis/Role/')
        .then(res=>{
            let data  = res.data;
            if (data['error'] === true){
    ...
    async function saveRole(){
        if (!isUpdate){
            return await axiosInstance.post('apis/Role/',{...fields})
                .then(res=>{
                    let data  = res.data;
                    if (data['error'] === true){
            ...
            return await axiosInstance.put(`apis/Role/${selectedObj.id}/`,{...fields})
                .then(res=>{
                    let data  = res.data;
                    if (data['error'] === true){
                        alert(`Error Occures ${data['message']}`);
    ...
    async function ConfirmDelete(){
        return await axiosInstance.delete(`apis/Role/${selectedObj.id}/`)
        .then(res=>{
            let data  = res.data;
            if (data['error'] === true){
                alert(`Error Occures ${data['message']}`);
    ...
    async function GetRoleFroUpdate(id = selectedObj.id){
        return await axiosInstance.get(`apis/Role/${id}/`)
        .then(res=>{
            let data  = res.data;
            if (data['error'] === true){
                alert(`Error Occures ${data['message']}`);
    ...
    return (
        <Grid container spacing={2} className={classes.formRoot}>
            ...
            <Grid item xs={10} lg={11}>     
                <Typography variant="h4" gutterBottom  color='primary'>Add Role</Typography>
            </Grid>
            {/* Left */}
            <Grid item xs={2} lg={1}>
