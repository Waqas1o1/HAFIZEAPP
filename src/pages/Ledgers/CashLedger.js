...
export default function CashLedger() {
    ...
    async function fetchLedger(){
        if (navigator.onLine){
            return await axiosInstance.get(`apis/CashLedger/${fields.FromDate}/${fields.ToDate}`)
            .then(res=>{
                let data  = res;
                if (data['error'] === true){
                    ...
                    setRows(data);
                    setLoading(false);
                    localStorage.removeItem('CashLedger');
                }
            })
            .catch(error=>{
                setLoading(false);
            ...
            setRows(JSON.parse(localStorage.getItem('CashLedger')));
        }
    }
    ...
    return (
        ...
        >
           {/* Title */}
           <Grid item xs={12} >     
                <Typography variant="h4" gutterBottom  color='primary'>Cash Ledger</Typography>
            </Grid>
