...
export default function ChequeLedger() {
    var date = new Date();
    const initialFields = {
    ...
    async function fetchLedger(){
        if (navigator.onLine){
            return await axiosInstance.get(`apis/ChequeLedger/${fields.FromDate}/${fields.ToDate}`)
            .then(res=>{
                let data  = res;
                if (data['error'] === true){
    ...
    return (
        ...
        >
           {/* Title */}
           <Grid item xs={12} >     
                <Typography variant="h4" gutterBottom  color='primary'>Cheque Ledger</Typography>
            </Grid>
